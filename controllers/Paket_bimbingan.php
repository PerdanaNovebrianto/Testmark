...
class Paket_bimbingan extends CI_Controller {
	...
	public function index(){
		if(isset($this->session->userdata['login_user'])){
            $data['paket'] = $this->Sahabat_Skripsi->get_all_guide();
			$data['judul_halaman'] = 'Kelola Bimbingan | Sahabat Skripsi';
			$data['pesan'] = $this->session->flashdata('message');
			$this->load->view('admin/templates/header', $data);
			$this->load->view('admin/paket_bimbingan', $data);
    ...
    public function tambah_paket(){
        if ($this->input->server('REQUEST_METHOD') == 'POST'){
            ...
            header('location:'.base_url().'paket_bimbingan');
        }
        else{
            show_404();
        }
